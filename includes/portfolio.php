 ...
 */
class Bluefrog_Addons_Portfolio {
	private $post_type = 'portfolio';
	private $taxonomy_type = 'portfolio_type';
	private $option = 'bluefrog_portfolio';
	...
	public function __construct() {
		...
		// Add image size
		add_image_size( 'bluefrog-portfolio', 451, 451, true );
		add_image_size( 'bluefrog-portfolio-large', 912, 912, true );
		add_image_size( 'bluefrog-portfolio-wide', 912, 451, true );
		add_image_size( 'bluefrog-portfolio-masonry', 565, 10000, false );
		add_image_size( 'bluefrog-portfolio-single', 1140, 740, true );
		...
		add_filter( 'bluefrog_breadcrumbs_args', array( $this, 'breadcrumb' ) );
	...
	public function register_post_type() {
		...
		$portfolio_permalink = empty( $permalinks['portfolio_base'] ) ? _x( 'portfolio', 'slug' ) : $permalinks['portfolio_base'];
		$portfolio_page_id   = get_option( $this->option . '_page_id' );
		$portfolio_type_base = empty( $permalinks['portfolio_type_base'] ) ? _x( 'portfolio-type', 'slug' ) : $permalinks['portfolio_type_base'];

		register_post_type( $this->post_type, array(
			'description'         => esc_html__( 'Portfolio Items' ),
			'labels'              => array(
				...
				'filter_items_list'     => esc_html__( 'Filter projects list' ),
				'items_list_navigation' => esc_html__( 'Project list navigation' ),
				'items_list'            => esc_html__( 'Projects list' ),
			),
			'supports'            => array(
				'title',
				'editor',
		...
		register_taxonomy( $this->taxonomy_type, $this->post_type, array(
			'hierarchical'      => true,
			'labels'            => array(
				'name'                  => esc_html__( 'Project Types' ),
				'singular_name'         => esc_html__( 'Project Type' ),
				'menu_name'             => esc_html__( 'Project Types' ),
				'all_items'             => esc_html__( 'All Project Types' ),
				'edit_item'             => esc_html__( 'Edit Project Type' ),
				'view_item'             => esc_html__( 'View Project Type' ),
				...
				'items_list_navigation' => esc_html__( 'Project type list navigation' ),
				'items_list'            => esc_html__( 'Project type list' ),
			),
			'public'            => true,
			'show_ui'           => true,
			'show_in_nav_menus' => true,
	...
	public function edit_admin_columns( $columns ) {
		...
		$columns['title'] = esc_html__( 'Project' );
	...
	public function settings_api_init() {
		add_settings_section(
			'bluefrog_portfolio_section',
			'<span id="portfolio-options">' . esc_html__( 'Portfolio' ) . '</span>',
		...
		add_settings_field(
			$this->option,
			'<span class="portfolio-options">' . esc_html__( 'Portfolio Projects' ) . '</span>',
		...
		if ( get_option( $this->option ) ) {
			...
			add_settings_field(
				...
				'<span class="portfolio-options">' . esc_html__( 'Portfolio page' ) . '</span>',
			...
			add_settings_field(
				...
				'<label for="portfolio_items_per_page">' . esc_html__( 'Portfolio items show at most' ) . '</label>',
			...
			add_settings_section(
				'bluefrog_portfolio_section',
				'<span id="portfolio-options">' . esc_html__( 'Portfolio Item Permalink' ) . '</span>',
			...
			add_settings_field(
				'portfolio_type_slug',
				'<label for="portfolio_type_slug">' . esc_html__( 'Portfolio type base' ) . '</label>',
	...
	public function writing_section_html() {
		?>
		<p>
			<?php esc_html_e( 'Use these settings to display custom types of content on your site' ); ?>
		</p>
		<?php
	}
	...
	public function reading_section_html() {
		?>
		<p>
			<?php esc_html_e( 'Use these settings to control custom post type content' ); ?>
		</p>
		<?php
	}
	...
	public function permalink_section_html() {
		...
		$portfolio_page_id = get_option( $this->option . '_page_id' );
		$base_slug         = urldecode( ( $portfolio_page_id > 0 && get_post( $portfolio_page_id ) ) ? get_page_uri( $portfolio_page_id ) : _x( 'portfolio', 'Default slug' ) );
		$portfolio_base    = _x( 'portfolio', 'Default slug' );
		...
		?>
		<p>
			<?php esc_html_e( 'Use these settings to control the permalink used specifically for portfolio.' ); ?>
		</p>

		<table class="form-table bluefrog-portfolio-permalink-structure">
			<tbody>
			<tr>
				<th>
					<label><input name="portfolio_permalink" type="radio"
								  ...
								  class="bluefrog-portfolio-base" /> <?php esc_html_e( 'Default' ); ?>
					</label>
				</th>
				<td>
				...
				<tr>
					<th>
						<label><input name="portfolio_permalink" type="radio"
									  ...
									  class="bluefrog-portfolio-base" /> <?php esc_html_e( 'Portfolio base' ); ?>
						</label>
					</th>
					<td>
			...
			<tr>
				<th>
					<label><input name="portfolio_permalink" type="radio"
								  ...
								  class="bluefrog-portfolio-base" /> <?php esc_html_e( 'Portfolio base with type' ); ?>
					</label>
				</th>
				<td>
			...
			<tr>
				<th>
					<label><input name="portfolio_permalink" id="bluefrog_portfolio_custom_selection" type="radio"
								  value="custom" <?php checked( in_array( $portfolio_permalink, $structures ), false ); ?> /> <?php esc_html_e( 'Custom Base' ); ?>
					</label>
				</th>
				<td>
					<code><?php echo esc_html( home_url() ); ?></code>
					<input name="portfolio_permalink_structure" id="bluefrog_portfolio_permalink_structure" type="text"
		...
		<script type="text/javascript">
			jQuery( function () {
				jQuery( 'input.bluefrog-portfolio-base' ).change( function () {
					jQuery( '#bluefrog_portfolio_permalink_structure' ).val( jQuery( this ).val() );
				} );
				jQuery( '.permalink-structure input' ).change( function () {
					jQuery( '.bluefrog-portfolio-permalink-structure' ).find( 'code.non-default-example, code.default-example' ).hide();
					if ( jQuery( this ).val() ) {
						jQuery( '.bluefrog-portfolio-permalink-structure code.non-default-example' ).show();
						jQuery( '.bluefrog-portfolio-permalink-structure input' ).removeAttr( 'disabled' );
					} else {
						jQuery( '.bluefrog-portfolio-permalink-structure code.default-example' ).show();
						jQuery( '.bluefrog-portfolio-permalink-structure input:eq(0)' ).click();
						jQuery( '.bluefrog-portfolio-permalink-structure input' ).attr( 'disabled', 'disabled' );
					}
				} );
				jQuery( '.permalink-structure input:checked' ).change();
				jQuery( '#bluefrog_portfolio_permalink_structure' ).focus( function () {
					jQuery( '#bluefrog_portfolio_custom_selection' ).click();
				} );
			} );
		</script>
		<?php
	...
	public function enable_field_html() {
		...
		<label for="<?php echo esc_attr( $this->option ); ?>">
			...
			<?php esc_html_e( 'Enable Portfolio Projects for this site.' ); ?>
		</label>
	...
	public function per_page_field_html() {
		...
		<label for="portfolio_posts_per_page">
			...
			<?php _ex( 'items', 'Portfolio items per page' ) ?>
		</label>
	...
	public function portfolio_type_slug_field_html() {
		...
		<input name="portfolio_type_slug" id="portfolio_type_slug" type="text"
			   ...
			   placeholder="<?php echo esc_attr( _x( 'portfolio-type', 'Portfolio type base' ) ) ?>"
		...
		if ( isset( $_POST['portfolio_permalink'] ) ) {
			...
			if ( 'custom' === $portfolio_permalink ) {
				...
				if ( '%portfolio_type%' == $portfolio_permalink ) {
					$portfolio_permalink = '/' . _x( 'portfolio', 'slug' ) . '/' . $portfolio_permalink;
				}
			...
			$portfolio_page_id   = get_option( $this->option . '_page_id' );
			$portfolio_permalink = ( $portfolio_page_id > 0 && get_post( $portfolio_page_id ) ) ? get_page_uri( $portfolio_page_id ) : _x( 'portfolio', 'Default slug' );
	...
	public function query_reading_setting( $query ) {
		...
		if ( $query->is_page() && 'page' === get_option( 'show_on_front' ) && $query->get( 'page_id' ) == get_option( 'page_on_front' ) && absint( $query->get( 'page_id' ) ) == get_option( $this->option . '_page_id' ) ) {
	...
	public function portfolio_post_type_link( $permalink, $post ) {
		...
		if ( ! empty( $terms ) ) {
			...
			$portfolio_type = _x( 'uncategorized', 'slug' );
		}
	...
	public function breadcrumb( $args ) {
		...
		if ( is_post_type_archive( 'portfolio' ) ) {
			...
			if ( $portfolio_page_id && get_post( $portfolio_page_id ) ) {
				...
				$args['labels']['archive'] = _x( 'Portfolio', 'Portfolio post type breadcrumb' );
			}
		}
	...
	public function archive_title( $title ) {
		if ( is_post_type_archive( 'portfolio' ) ) {
			...
			if ( $portfolio_page_id && get_post( $portfolio_page_id ) ) {
				...
				$title = _x( 'Portfolio', 'Portfolio post type breadcrumb' );
			}
		}
