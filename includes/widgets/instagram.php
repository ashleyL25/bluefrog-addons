...
if ( ! class_exists( 'Bluefrog_Instagram_Widget' ) ) :

/**
 * Instagram widget class
 */
class Bluefrog_Instagram_Widget extends WP_Widget {
	/**
	...
	public function __construct() {
		...
		parent::__construct(
			'instagram-feed-widget',
			esc_html__( 'Bluefrog - Instagram' ),
			array(
				'classname'                   => 'instagram-feed-widget',
				'description'                 => esc_html__( 'Displays your latest Instagram photos' ),
	...
	public function form( $instance ) {
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'token' ) ); ?>"><?php esc_html_e( 'Access Token' ); ?></label>
			<input class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'token' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'token' ) ); ?>" type="text" value="<?php echo esc_attr( $instance['token'] ); ?>" placeholder="<?php esc_attr_e( 'Access Token' ) ?>">
		</p>

		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'number' ) ); ?>"><?php esc_html_e( 'Number of Photos' ); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'columns' ) ); ?>"><?php esc_html_e( 'Columns' ); ?></label>
			<select id="<?php echo esc_attr( $this->get_field_id( 'columns' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'columns' ) ); ?>" class="widefat">
				<option value="1" <?php selected( '1', $instance['columns'] ); ?>><?php esc_html_e( '1 Column' ); ?></option>
				<option value="2" <?php selected( '2', $instance['columns'] ); ?>><?php esc_html_e( '2 Columns' ); ?></option>
				<option value="3" <?php selected( '3', $instance['columns'] ); ?>><?php esc_html_e( '3 Columns' ); ?></option>
				<option value="4" <?php selected( '4', $instance['columns'] ); ?>><?php esc_html_e( '4 Columns' ); ?></option>
			</select>
		</p>
	...
	protected function scrape_instagram( $username ) {
		...
		$transient_key = 'bluefrog_instagram-' . $transient_prefix . '-' . sanitize_title_with_dashes( $username );
	...
	public function get_photos_by_token( $access_token, $limit = 12 ) {
		if ( ! function_exists( 'bluefrog_fetch_instagram_media' ) ) {
			return new WP_Error( 'not_bluefrog', esc_html__( 'This widget works with the Bluefrog theme only.' ) );
		}

		$transient_key = 'bluefrog_instagram_photos' . sanitize_title_with_dashes( $access_token ) . '_' . $limit;
		...
		if ( false === $images || empty( $images ) ) {
			...
			while ( count( $images ) < $limit ) {
				if ( ! $next ) {
					$fetched = bluefrog_fetch_instagram_media( $access_token );
				} else {
					$fetched = bluefrog_fetch_instagram_media( $next );
				}
		...
		if ( ! empty( $images ) ) {
			...
			return new WP_Error( 'instagram_no_images', esc_html__( 'Instagram did not return any images.' ) );
		}
	}
	...
	public function display_media( $media, $as_background = false, $tag = 'li' ) {
		if ( function_exists( 'bluefrog_instagram_image' ) ) {
			bluefrog_instagram_image( $media, $as_background, $tag );
			return;
		}
